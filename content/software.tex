...
\begin{itemize}
    ...
    \item \textbf{Acciones actuadoras}: Son aquellas que interactúan con el entorno en el
    ...
    o para cambiar el estado del agente. Esto permite al agente adaptarse a entornos cambiantes.
...
debido a problemas de incompatibilidad o permisos con distintas distribuciones de Linux. 
...
Esta funcionalidad, se envolverá dentro de una función única para todos los sistemas operativos, 
y permitirá al agente escribir texto en la máquina esclava de forma autónoma:
...
ha creado un diccionario de combinaciones de teclas que incluyen múltiples representaciones
de una misma combinación o tecla, como puede ser \texttt{CTRL}, \texttt{CONTROL} o \texttt{CTRL\_LEFT}.
...
espacio de acciones elementales \(\mathbb{B}\). Esto se debe a que múltiples aplicaciones 
...
atravesará por varios módulos de OCR (Reconocimiento Óptico de Caracteres),
...
\begin{figure}[!h]
	...
	\caption{\small Procesamiento de una captura de pantalla con algoritmos de detección de elementos.}
	\label{fig:ocr_image}
\end{figure}

Nótese que aunque esta acción es perceptiva, tambien forma parte de las acciones
elementales \(\mathbb{B}\) del espacio de acciones, ya que múltiples 
...
recibir una condición dada en lenguaje natural, y analizará periódicamente
...
lenguaje natural a una búsqueda de aplicaciones en el sistema operativo esclavo.

Para realizar esto, se realiza una búsqueda en múltiples directorios con usualmente
...
Nótese que aunque esta acción puede ser completada únicamente mediante acciones elementales
(clicks y escritura de texto), su implementación permite una interacción mas ágil y eficiente
con el sistema. Formalmente, reemplazamos una solución \(s_{sub}\) con una solución \(s_{opt}\)
...
\begin{figure}[!b]
	...
	\caption{\small Interfaz gráfica de seguridad para ejecución de código.}
...
\begin{itemize}
    ...
    Estas tareas siempre se asegura que están definidas siguiendo el protocolo
...
\begin{itemize}
    ...
    a su vez mediante una única función Python, cada acción estará compuesta por
    ...
    un agente puede decidir parar un análisis si recibe una notificación de un
    evento importante.
    
    El identificador es generado automáticamente por el intérprete y permite a
...
normal. Dentro de esta tarea, vemos como distintos tipos de secuencia permiten
...
creado puramente en Python (denominado como Pyxcel, del juego de palabras
"Py-thon E-xcel-ent") que enlaza las acciones \emph{just-in-time}, es
...
el código Python correspondiente a cada una de ellas. De esta forma, el intérprete
...
\begin{itemize}
    ...
    podrá buscar en el mapa de correspondencia \(\Lambda\) y generar código exelent que
...
de forma efectiva, generando código exelent siguiendo las instrucciones embebidas en el autodescriptor.
...
>>   (
>>     "user",
>>     "Por favor escribe 'Hola Mundo' en la maquina 
>>      esclava y luego haz click en cerrar ventana"
>>   ),
>>   (
>>     "environment",
>>     "La maquina esclava esta en el escritorio"
>>   ),
>>   (
>>     "action",
>>     "start writing hello world with priority normal"
...
ya no son relevantes para las próximas tareas. Así, el buffer de memoria
...
>>   (
>>     "user",
>>     "Por favor escribe 'Hola Mundo' en la maquina esclava 
...
Adicionalmente, el buffer de memoria también contiene mecanismos para minimizar
...
los agentes mantendrán un estado coherente a lo largo de la ejecución de una tarea,
...
el agente. Cuando un agente realiza una meta-acción, esta modifica su estado cognitivo, permitiéndole
...
embeber todas las capacidades de un agente en una única función de Python.

A su vez, este protocolo es totalmente síncrono y basado en un sistema de mensajes, donde
al enviar un mensaje a un agente que utiliza el protocolo FastAP, este tomará el control
...
\begin{itemize}
    ...
    necesarias para cada agente. Una vez con estas dependencias, FastAP genera un entorno virtual
    ...
    envíe mensajes de confirmación para continuar con la ejecución de su tarea. Cuando una señal de 
...
que puede ser fácilmente adaptada a los mecanismos internos de cada agente.
...
\begin{itemize}
    ...
    como interdependencias entre acciones. Por ejemplo, si una acción de reconocimiento de
    ...
    es teóricamente tendiente a infinito, por lo que en caso de que la carga no fuera perezosa, el sistema
    ...
    \item \textbf{Mapa de Correspondencia}: El registro guarda un mapa de correspondencia \(\Lambda\) basado en múltiples
    ...
    \item \textbf{Entorno Maestro-Esclavo}: El registro permite crear un entorno maestro-esclavo para dividir la MCE en dos máquinas:
    ...
    funcionalidades comunes. Estos paquetes agilizan la resolución de dependencias y a su vez permiten a los agentes simplificar la invocación 
    ...
    \item \textbf{Ejecución Segura}: El registro también permite a la MCE ejecutar acciones de forma segura,
...
Este servicio se realizó a través de una implementación basada en RabbitMQ con
mensajes de tipo string entre ambas máquinas. Debido a la naturaleza de la MCE, 
se requiere que la ejecución de código en una máquina remota sea lo más segura 
...
Nótese que múltiples mecanismos internos han sido omitidos para
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item DarkVFR inicializa los módulos de la capa de cognición necesarios, como por ejemplo,
        ...
        \item FastAP enlaza al agente (DarkVFR) con el interprete, permitiéndole enviar acciones a
...
>>         open_application "Spotify"
        \end{lstlisting}
        \item DarkVFR envía el plan de acción a la capa de ejecución a través del interprete.
    \end{enumerate}

    \item El interprete recibe el plan de acción y lo envía a la capa de ejecución.
    \begin{enumerate}
        ...
        y sus metadatos asociados a través de su identificador único y el mapa de correspondencia \(\Lambda\).
        ...
        \item El registro de entidades es disparado con la acción \texttt{open\_application} y carga todas las librerías
    ...
    \item La MCE recibe el flag de tarea completada y finaliza la ejecución de la tarea.
    \begin{enumerate}
        \item El registro de entidades revisa si hay librerías que pueden ser liberadas, y si es así, las elimina.
        \item FastAP deja el entorno virtual del agente bloqueado hasta próximo aviso.
...
\begin{itemize}
    ...
    para próximas ejecuciones.\footnote{En versiones previas del espacio de ejecución
...
\begin{figure}[!h]
	...
	\caption{\small Interfaz gráfica de desarrollador}
	\label{fig:installer}
\end{figure}
...
Ambas imágenes han sido desarrolladas desde la placa de desarrollo
IMX8MP de NXP, con un microprocesador ARM Cortex A53 de 4 núcleos. No
se asegura que estas imágenes sean compatibles en su totalidad con otros 
...
arquitectura de red estándar para comunicaciones a través de, por ejemplo,
...
Nótese como a esta imagen no se le ha integrado una interfaz gráfica o gestor de ventanas, 
...
\begin{itemize}
    ...
    que la MCE puede ser arrancada correctamente, utilizando distintos componentes y 
    configuraciones.
