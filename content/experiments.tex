...
\begin{itemize}
    ...
    fase consiste en la generación de lenguaje exelent para enviarlo
    ...
    adicional de recuperación. En este sentido, cuando el agente genera alucinaciones
    ...
    Este mecanismo permite al agente recuperarse de forma más ágil, sin tener que
    ...
    una filosofía de planificación basada en \emph{ReAct}, una de las metodologías
    ...
    a largo plazo, o generar lenguaje exelent para enviarlo a la capa de ejecución.
    ...
    de forma reactiva, sino también modificar su comportamiento a largo plazo a medida
    ...
    que permiten al agente razonar y actuar de forma paralela y mucho más eficiente.
    ...
    desde el espacio de acciones, sino también desde imágenes o capturas de pantalla 
    del entorno.
    ...
    separados y reemplazables que pueden ser sobrescritos para extender su comportamiento.
...
así como módulos customizados para cada agente escritos totalmente en Python.
...
\begin{enumerate}
  ...
  \item \textbf{Play a Song}: En este entorno, el agente tendrá Spotify
  ...
  en el escritorio de Windows. Tendrá aplicaciones abiertas como
  Spotify o Visual Studio Code, pero no tendrá ningún navegador abierto.
  ...
  se medirá tanto en el número de acciones como en la cantidad de tokens
  ...
  La entrada que recibirá el agente para esta prueba será:
  \begin{quote}
    \texttt{Busca un portátil en Amazon y cómpralo o añádelo al carrito.}
  \end{quote}

\end{enumerate}

Nótese que como medida de seguridad, los agentes contarán con un limite
...
\begin{itemize}
    ...
    para abrir un editor o revisar si en pantalla se encuentra algún programa
...
con un número reducido de acciones, utilizando una estrategia más
...
\begin{figure}[!h]
    ...
    \caption{Resultado de la evaluación \emph{Hello World} usando Planex. Texto escrito resaltado en un recuadro rojo}
...
\begin{itemize}
    ...
    abrir Spotify directamente, mientras que en otras ejecuciones han encontrado
    ...
    sin embargo este obstáculo resulta un bloqueo que ha impedido el completado
...
\begin{figure}[!h]
    ...
    \caption{VisualFastReact maximizando Spotify para reproducir la canción}
...
Adicionalmente como se muestra en la figura \ref{fig:laptops}, en búsquedas realizadas por los agentes, 
se ha observado como incluso habiendo estos utilizado la búsqueda \emph{laptops site:amazon.com}, los resultados
de la búsqueda no contienen únicamente productos de Amazon, sino que son mayoritariamente anuncios de 
...
\begin{figure}[!h]
    ...
    \caption{Búsqueda de portátil usando el query \emph{laptops site:amazon.com} en Microsoft Edge. 
    Los resultados de la búsqueda únicamente contienen un producto perteneciente a Amazon, el resto
    son anuncios de otras páginas}
    \label{fig:laptops}
\end{figure}
...
\begin{itemize}
  ...
  búsqueda en Edge, sin embargo han tenido problemas al no ser capaces de distinguir
  ...
  la página de búsqueda de portátiles de Amazon.
...
los obstáculos de descubrimiento del navegador y de interacción con el mismo.
...
En el caso de \emph{DarkVFR}, el agente obtiene un promedio récord de 
...
Como puede observarse, los agentes \emph{Planex} y \emph{PlanexV2} presentan
...
Por el contrario, los agentes \emph{Xplore} y aquellos basados en \emph{FastReact} 
fueron capaces de completar satisfactoriamente los tres entornos, aunque con
diferencias relevantes en el tiempo de ejecución. \emph{Xplore} muestra
...
exhaustiva y posiblemente redundante. En comparación, \emph{FastReact} logra
una mayor eficiencia con un tiempo medio de 305.46 segundos en ese mismo
entorno.

Finalmente, destaca el rendimiento de \emph{DarkVFR}, que no solo completa
...
optimización en agentes como \emph{DarkVFR} es determinante para un
...
Entre todos los agentes evaluados, \emph{DarkVFR} destaca no solo por ser el
...
decir, entre 7 y 12 veces más costoso que \emph{DarkVFR}. Este resultado
...
caso de \emph{Planex}, \emph{PlanexV2} y \emph{RePlan}, los errores están
...
Por otro lado, los agentes \emph{Xplore} y \emph{FastReact} presentan fallos
...
En contraste, los agentes \emph{V-FastReact} y \emph{DarkVFR} han sido
